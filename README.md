...
| `entrypoint.py` | 通用入口；按操作直接启动训练或评估脚本 | 是 | 否 |
| `algorithm/` | 直接读取JSON配置的单标签训练和评估算法 | 是 | 否 |
...
新增任务时，在`docker/`下创建平级的独立镜像目录，让其`train.py`和`test.py`直接支持`--config`即可；通用`entrypoint.py`无需感知任务配置字段。随后在`image-map.json`增加映射，不要向现有单标签镜像混入其他任务的依赖。
